...
public class PersonaService {
	...
	public Respuesta guardar(Persona persona){
		...
		try {
			if (dao.existsById(persona.getCurp())) {
				rs.setMensaje("La persona no se agrego a la base de datos porque ya existe");
				rs.setSuccess(false);
	...
	public Respuesta editar(Persona persona) {
		...
		try {
			...
			rs.setMensaje("La persona que tratas de buscar no existe");
			rs.setSuccess(false);
			rs.setObj(persona.getCurp());
			return rs;
			
		} catch (Exception e) {
			rs.setMensaje("Error al intentar editar, intente mas tarde");
			rs.setSuccess(false);
			rs.setObj(persona.getCurp());
		}
	...
	public Respuesta elimiar(Persona persona) {
		Respuesta rs = new Respuesta();
